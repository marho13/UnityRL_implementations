...
class EpsilonGreedyStrategy():
    ...
    def get_exploration_rate(self, current_step):
        result = self.start ** (current_step/self.decay)
        if result<self.end:
            return self.end
        else:
            return result
...
class policyNetworks(nn.Module):
    def __init__(self, stateDim, outputSize, n_latent_var):
        super().__init__()
        self.strategy = EpsilonGreedyStrategy(0.99, 0.05, 200)
        # self.device = device
        self.randPolicy = {"Rand":0, "Policy":0}
        self.current_step = 3000
        self.num_actions = outputSize
        self.fc1 = nn.Linear(in_features=stateDim, out_features=n_latent_var).type(torch.FloatTensor)
        self.fc2 = nn.Linear(in_features=n_latent_var, out_features=n_latent_var).type(torch.FloatTensor)
        self.out = nn.Linear(in_features=n_latent_var, out_features=outputSize).type(torch.FloatTensor)


    def forward(self, t):
        t = t.flatten().type(torch.FloatTensor)
        t = self.fc1(t).type(torch.FloatTensor)
        t = F.relu(t).type(torch.FloatTensor)
        t = self.fc2(t).type(torch.FloatTensor)
        t = F.relu(t).type(torch.FloatTensor)
        t = self.out(t)
        return t
        ...
        if rate > random.random():
            ...
            for a in range(self.num_actions):
                if a != (action):
                    ...
                    output.append(1.0)
            return torch.tensor(output).to('cpu') # explore
        else:
            self.randPolicy["Policy"] += 1
            state = torch.tensor(state, dtype=torch.float32)
            # state = torch.from_numpy(state)
            with torch.no_grad():
                return self.forward(state).to('cpu') # exploit
